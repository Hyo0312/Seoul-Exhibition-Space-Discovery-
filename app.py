...
.prd-card { background: #ffffff; border: 1px solid #ddd5c8; border-radius: 14px; padding: 22px 24px; box-shadow: 0 2px 12px rgba(80,55,35,.07); height: 100%; }
...
.step-card { background: #ffffff; border: 1px solid #ddd5c8; border-radius: 12px; padding: 18px 20px; display: flex; gap: 16px; align-items: flex-start; margin-bottom: 12px; box-shadow: 0 2px 8px rgba(80,55,35,.05); }
.step-num { width: 32px; height: 32px; border-radius: 50%; background: rgba(196,113,74,.12); border: 1.5px solid rgba(196,113,74,.3); display: flex; align-items: center; justify-content: center; font-size: 13px; color: #c4714a; font-weight: 500; flex-shrink: 0; }
.tech-pill { display: inline-block; background: rgba(122,158,142,.13); color: #7a9e8e; border: 1px solid rgba(122,158,142,.3); border-radius: 4px; font-size: 11px; padding: 4px 12px; margin: 3px 4px 3px 0; letter-spacing: .08em; }
</style>
...
NEIGHBORHOODS = {
    "Seongsu":   {"vibe": "🏭 Industrial Chic",  "atm": 4.5, "acc": 4.3, "pop": 4.6, "avg_price": 3000, "best": "아트+커피 데이트",    "hex": "#c4714a"},
    "Bukchon":   {"vibe": "⛩️ Historic Calm",    "atm": 4.8, "acc": 3.9, "pop": 4.5, "avg_price": 2000, "best": "전통 문화 탐방",      "hex": "#7a9e8e"},
    "Itaewon":   {"vibe": "🌍 Cosmopolitan",      "atm": 4.2, "acc": 4.5, "pop": 4.3, "avg_price": 5000, "best": "국제 아트 씬",        "hex": "#b8924a"},
    "Hannam":    {"vibe": "🎨 Refined Luxury",    "atm": 4.6, "acc": 4.3, "pop": 4.4, "avg_price": 0,    "best": "갤러리 호핑",         "hex": "#d4856e"},
    "Hongdae":   {"vibe": "🎸 Youthful Energy",   "atm": 4.0, "acc": 4.7, "pop": 4.8, "avg_price": 3000, "best": "인디·스트리트 아트",  "hex": "#8c78c3"},
    "Insadong":  {"vibe": "🖼️ Artisan Soul",      "atm": 4.4, "acc": 4.5, "pop": 4.6, "avg_price": 0,    "best": "공예·문화 체험",      "hex": "#64a078"},
    "Samcheong": {"vibe": "🌸 Quiet Elegance",    "atm": 4.7, "acc": 4.2, "pop": 4.2, "avg_price": 0,    "best": "파인 아트 갤러리",    "hex": "#a082c3"},
    "Yongsan":   {"vibe": "🏛️ Urban Grand",      "atm": 4.5, "acc": 4.7, "pop": 4.6, "avg_price": 2000, "best": "국립 박물관",          "hex": "#c39b64"},
}
...
if page == "🗺️ Overview":
    ...
    with col_l:
        ...
        color_map = {a: NEIGHBORHOODS[a]["hex"] for a in nb_counts["area"]}
        fig_bar = px.bar(
            nb_counts, x="area", y="count",
            color="area", color_discrete_map=color_map,
            labels={"area": "동네", "count": "공간 수"},
        )
        fig_bar.update_layout(**BASE_LAYOUT, height=280, showlegend=False)
        fig_bar.update_traces(marker_line_width=0)
        fig_bar.update_xaxes(gridcolor=GRID_COL)
    ...
    with col_r:
        ...
        fig_pie = px.pie(
            mood_counts, names="label", values="count", hole=0.5,
            color_discrete_sequence=["#7a9e8e","#c4714a","#b8a0d0","#d4856e","#b8924a"],
        )
        fig_pie.update_layout(**BASE_LAYOUT, height=280)
        st.plotly_chart(fig_pie, use_container_width=True)

    # ── 가격 분포 ──
    st.markdown("#### 💳 입장료 분포")
    price_bins   = ["🎟️ Free", "₩1–5k", "₩5–10k", "₩10–20k", "₩20k+"]
    price_counts = [
        len(df[df["price"] == 0]),
    ...
    fig_price = px.bar(
        x=price_bins, y=price_counts,
        color=price_bins,
        color_discrete_sequence=["#7a9e8e","#b8924a","#c4714a","#d4856e","#b8a0c8"],
        labels={"x": "가격대", "y": "공간 수"},
    )
    fig_price.update_layout(**BASE_LAYOUT, height=260, showlegend=False)
    fig_price.update_traces(marker_line_width=0)
    fig_price.update_xaxes(gridcolor=GRID_COL)
    fig_price.update_yaxes(gridcolor=GRID_COL)
...
# ════════════════════════════════════════
    ...
    selected = st.multiselect("무드를 선택하세요 (복수 선택 가능)", mood_options, default=["🌿 Healing"])
...
# ════════════════════════════════════════
    ...
    selected_nbs = st.multiselect("비교할 동네를 선택하세요", all_nbs, default=["Seongsu","Bukchon","Itaewon","Hannam"])
    ...
    if len(selected_nbs) < 2:
        ...
        with col_l:
            ...
            for nb in selected_nbs:
                ...
                fig_radar.add_trace(go.Scatterpolar(
                    ...
                    name=nb,
                    line_color=d["hex"],
                    fillcolor=hex_to_rgba(d["hex"], 0.12),
                ))
            fig_radar.update_layout(
                polar=dict(
                    bgcolor=PLOT_BG,
                    radialaxis=dict(visible=True, range=[3.5, 5], tickfont=dict(color="#a8998a"), gridcolor=GRID_COL),
                    angularaxis=dict(tickfont=dict(color=FONT_COL), gridcolor=GRID_COL),
                ),
                paper_bgcolor=PAPER_BG,
                font=dict(color=FONT_COL),
                legend=dict(font=dict(color=FONT_COL)),
        ...
        with col_r:
            ...
            for nb in selected_nbs:
                ...
                fig_scatter.add_trace(go.Scatter(
                    ...
                    marker=dict(size=18, color=d["hex"], line=dict(width=2, color="#ffffff")),
                ))
            fig_scatter.update_layout(
                **BASE_LAYOUT,
                xaxis=dict(title="Accessibility", range=[3.4, 5.1], gridcolor=GRID_COL),
                yaxis=dict(title="Popularity",    range=[3.4, 5.1], gridcolor=GRID_COL),
        ...
        for nb in selected_nbs:
            ...
            rows.append({
                "동네": nb, "분위기": d["vibe"],
                "Atmosphere ⭐": d["atm"], "Accessibility 🚇": d["acc"], "Popularity 🔥": d["pop"],
        ...
        nb_spaces = [s for s in SPACES if s["area"] in selected_nbs]
        col1, col2 = st.columns(2)
        for i, s in enumerate(nb_spaces):
            with (col1 if i % 2 == 0 else col2):
                render_card(s)
...
# ════════════════════════════════════════
    ...
    with col_sl:
        budget = st.slider("최대 예산 (₩)", 0, 30000, 15000, step=1000, format="₩%d")
    with col_mood:
        mood_filter = st.selectbox("무드 필터", ["모든 무드"] + [f"{MOOD_EMOJI[m]} {m}" for m in MOOD_EMOJI])
    ...
    with col_l:
        ...
        labels_th  = ["Free" if t == 0 else f"₩{t:,}" for t in thresholds]
        counts_th  = [len([s for s in SPACES if s["price"] <= t]) for t in thresholds]
        fig_line = px.line(
            x=labels_th, y=counts_th, markers=True,
            labels={"x": "예산", "y": "공간 수"},
            color_discrete_sequence=["#c4714a"],
        )
        fig_line.update_traces(line_width=2.5, marker_size=8)
        fig_line.update_layout(**BASE_LAYOUT, height=260)
        fig_line.update_xaxes(gridcolor=GRID_COL)
        fig_line.update_yaxes(gridcolor=GRID_COL)
    ...
    with col_r:
        ...
        fig_tier.update_layout(**BASE_LAYOUT, height=260, showlegend=False)
        fig_tier.update_traces(marker_line_width=0)
        fig_tier.update_xaxes(gridcolor=GRID_COL)
        fig_tier.update_yaxes(gridcolor=GRID_COL)
        st.plotly_chart(fig_tier, use_container_width=True)

    budget_label = "무료 공간만" if budget == 0 else f"₩{budget:,} 이하"
    st.success(f"✅ **{budget_label}** 기준 — **{len(filtered)}개** 공간을 이용할 수 있어요!")
    col1, col2 = st.columns(2)
    for i, s in enumerate(sorted(filtered, key=lambda x: x["price"])):
        with (col1 if i % 2 == 0 else col2):
            render_card(s)
...
# ════════════════════════════════════════
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("""<div class="prd-card"><h4>❓ Why — 프로젝트 목적</h4>
        <p>인기 순위가 아닌 무드·분위기·예산에 맞는 서울의 전시·문화 공간을 찾을 수 있도록 돕기 위한 프로젝트예요.
        전시 데이트 코스를 고민하는 사람들에게 데이터 시각화를 통한 개인화된 추천을 제공합니다.</p></div>""",
        unsafe_allow_html=True)
    with c2:
        st.markdown("""<div class="prd-card"><h4>👥 Who — 대상 사용자</h4><ul>
        <li>🧑‍🎨 20대 청년 및 전시 애호가</li>
        <li>💑 데이트 코스를 찾는 커플</li>
        <li>🌸 감각적인 공간에 관심 있는 사람</li>
        <li>🗺️ 서울 문화·예술 씬이 궁금한 누구나</li></ul></div>""",
        unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    with c3:
        st.markdown("""<div class="prd-card"><h4>🛠️ What — 핵심 기능</h4><ul>
        <li>🌿 무드 기반 추천 시스템</li>
        <li>🏘️ 동네 비교 툴 (성수, 북촌, 이태원, 한남…)</li>
        <li>💰 예산 기반 추천</li>
        <li>📊 인터랙티브 데이터 시각화</li></ul></div>""",
        unsafe_allow_html=True)
    with c4:
        st.markdown("""<div class="prd-card"><h4>⚙️ How — 기술 스택</h4>
        <p><span class="tech-pill">Python 3.9+</span><span class="tech-pill">Streamlit</span>
        <span class="tech-pill">Pandas</span><span class="tech-pill">Plotly</span>
        <span class="tech-pill">GitHub</span><span class="tech-pill">Streamlit Cloud</span></p>
        <p style="margin-top:12px">Python과 Streamlit 라이브러리를 활용하고, 전시 장소·문화 공간·예산 정보를 담은 큐레이션 데이터셋을 사용합니다.</p></div>""",
        unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("#### 🗓️ 3-Step Build Plan")
    for num, title, desc in [
        ("1", "🏗️ Basic UI Layout & Sample Dataset Setup",
         "GitHub 저장소 생성 및 Streamlit 환경 설정. 서울 전시·문화공간 샘플 데이터셋 준비. 홈화면 및 네비게이션 레이아웃 디자인."),
        ("2", "🔮 Interactive Recommendation & Visualization",
         "무드 기반 필터 구현. 예산 및 동네 비교 기능 추가. Plotly를 활용한 차트 시각화 적용."),
        ("3", "✨ UI Improvement & Real Data Integration",
         "깔끔하고 모던한 디자인 테마 적용. 실제 전시·문화공간 데이터 추가. 반응형 개선 및 배포 완료."),
    ]:
        st.markdown(f"""<div class="step-card">
            <div class="step-num">{num}</div>
            <div><strong>{title}</strong><br>
            <span style="font-size:13px;color:#6b5e52">{desc}</span></div>
        </div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""<div class="prd-card"><h4>💃 Academic & Creative Value</h4>
    <p>무용 전공자로서 공간·분위기·감정 경험의 관계에 대한 관심이 담긴 프로젝트입니다.
    전시 및 문화공간 데이터를 인터랙티브 추천 시스템으로 구성함으로써, 주관적 감각을 데이터 기반 문화 경험으로
    전환하는 새로운 방식으로 서울의 예술·전시 씬을 탐색할 수 있도록 합니다. 🌿</p></div>""",
    unsafe_allow_html=True)
